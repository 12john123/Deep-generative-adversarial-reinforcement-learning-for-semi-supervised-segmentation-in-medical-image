...
class liverset(Dataset):
    def __init__(self,anFilePath,labelFilePath,is_augment) -> None:
        super().__init__()
        anImgs=os.listdir(anFilePath)
        #notanImgs=os.listdir(notanFilePath)
        lableImgs=os.listdir(labelFilePath)
    ...
    def __getitem__(self, index):
        ...
        DataImgPath=self.anlist[index]
        #LabelImgPath=self.labellist[index]
        LabelImgPath=DataImgPath.replace('image','label')  #.replace('.png','_mask.png') #self.labellist[index]
        if(LabelImgPath.find('brain')!=-1 or LabelImgPath.find('pancreas')!=-1):
            LabelImgPath=LabelImgPath.replace('.png','_mask.png')
        
        pil_Dimg=cv2.imread(DataImgPath)[:,:,-1] #.transpose(1,0)
        #print(DataImgPath)
        #print(LabelImgPath)
        pil_Limg=cv2.imread(LabelImgPath)[:,:,-1] #cv2.imread(DataImgPath.replace('CT_','M_').replace('annotated','label'))[:,:,-1]*255
        #pil_Limg=cv2.flip(pil_Limg,1)
